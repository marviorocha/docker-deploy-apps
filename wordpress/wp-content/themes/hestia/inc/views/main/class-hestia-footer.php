	...
	public function bottom_footer_content() {
		$hestia_general_credits = get_theme_mod(
			...
			sprintf(
				...
				esc_html__( '%1$s | Hosted by %2$s', 'showbiz' ),
				esc_html__( 'Showbiz.mus.br', 'showbiz' ),
				/* translators: %1$s is URL, %2$s is WordPress */
				sprintf(
					'<a target="_blank" href="https://m.do.co/c/6493f9ca67ee" rel="nofollow">%2$s</a>',
					esc_url( __( 'Digital Ocean ', 'showbiz' ) ),
					'Digital Ocean'
				)
			)
		);
